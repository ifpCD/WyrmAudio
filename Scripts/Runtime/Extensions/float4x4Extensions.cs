...
internal static class Float4x4Extensions
{
    ...
    public static float4 GetRow(this float4x4 m, int index)
    {
        switch (index)
        {
            case 0: return math.float4(m.c0.x, m.c1.x, m.c2.x, m.c3.x);
            case 1: return math.float4(m.c0.y, m.c1.y, m.c2.y, m.c3.y);
            case 2: return math.float4(m.c0.z, m.c1.z, m.c2.z, m.c3.z);
            case 3: return math.float4(m.c0.w, m.c1.w, m.c2.w, m.c3.w);
    ...
    public static void SetRow(this ref float4x4 m, int index, float4 row)
    {
        switch (index)
        {
            case 0: m.c0.x = row.x; m.c1.x = row.y; m.c2.x = row.z; m.c3.x = row.w; break;
            case 1: m.c0.y = row.x; m.c1.y = row.y; m.c2.y = row.z; m.c3.y = row.w; break;
            case 2: m.c0.z = row.x; m.c1.z = row.y; m.c2.z = row.z; m.c3.z = row.w; break;
            case 3: m.c0.w = row.x; m.c1.w = row.y; m.c2.w = row.z; m.c3.w = row.w; break;
            default: throw new IndexOutOfRangeException();
        }
    }
}